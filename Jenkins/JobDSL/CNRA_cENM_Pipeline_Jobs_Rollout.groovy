pipelineJob('cENM_Design_Teams_Install') {
  description ('ENM Cloud Native Design Teams Install pipeline - autogenerated using JobDSL - all manual changes will be overwritten!')
  concurrentBuild(allowConcurrentBuild = true)
    ...
    parameters {
      ...
      choiceParam('use_case', ['Install', 'Upgrade', 'Teardown'], 'Select the appropriate operation to be done.')
      ...
      stringParam('snapshot_integration_key_value_pairs', '','Provide the snapshot integration key value pairs')
      ...
      choiceParam('BRO_DEFAULT_BACKUP_TYPE', ['Internal', 'External'], 'If External is selected then cenmproxy should be up and running on your environment nad backup will be done from ADPBRO. If Internal is selected then backup will be done from BROCLI.')
      stringParam('environment_name', '','The deployment name to run the job against')
      labelParam('slave') {
        defaultValue("${CNRA_Pipeline_Build_Agents}")
        description('')
      }
    }
    definition {
...
pipelineJob('cENM_Design_Teams_Uninstall') {
  description ('ENM Cloud Native Design Teams Uninstall pipeline - autogenerated using JobDSL - all manual changes will be overwritten!')
  concurrentBuild(allowConcurrentBuild = true)
    logRotator {
      numToKeep(100)
    }
...
pipelineJob('cENM_Design_Teams_Upgrade') {
  description ('ENM Cloud Native Design Teams Upgrade pipeline - autogenerated using JobDSL - all manual changes will be overwritten!')
  concurrentBuild(allowConcurrentBuild = true)
    logRotator {
      numToKeep(100)
    }
